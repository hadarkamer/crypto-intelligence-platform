...
- Weekend/weekday regime is a first-class analytical variable. Crypto weekend trading is commonly thinner, so compare Price/OI/Futures CVD/Spot CVD changes with older observations for the same symbol and the same UTC weekend/weekday regime. Do not assume that a smaller weekend move is automatically good; report both absolute movement and same-regime percentile.
- When historical_context is available, prefer its prior-only same-regime percentiles over comparing an absolute raw value across unlike symbols or across weekend and weekday. The baseline excludes the current observation and all future observations.
