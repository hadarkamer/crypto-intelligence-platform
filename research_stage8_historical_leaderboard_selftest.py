...
def _route_population(binding_sha: str, route: str, included: list[int],
...
def _evaluation(binding_sha: str, *, successes: int = 0, failures: int = 0,
    ...
    probability_ids = list(range(1001, 1001 + probability_n))
    asymmetry_ids = list(range(2001, 2001 + asymmetry_n))
...
class Stage8HistoricalLeaderboardTests(unittest.TestCase):
    ...
    def test_route_parent_identity_and_exclusion_provenance_is_fail_closed(self):
        ...
        for mutate, message in (
            (lambda route: route.update(included_btc_parent_movement_ids=[1001, 1001]), "sorted, unique"),
            (lambda route: route.update(route_population_sha256="0" * 64), "digest mismatch"),
        ):
            invalid = deepcopy(evaluation)
            mutate(invalid["routes"]["PROBABILITY"])
            with self.assertRaisesRegex(ValueError, message):
                _build({binding_sha: invalid})
        valid = deepcopy(evaluation)
        route = valid["routes"]["PROBABILITY"]
        route["excluded_btc_parents"] = [{
            "btc_parent_movement_id": 9001, "reason_code": "NO_DECISIVE_LABEL",
        }]
