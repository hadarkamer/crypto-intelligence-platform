...
1. Apply migrations `001` through `056` to a staging PostgreSQL 15+ database
...
schema containing migration 056 (normally `public`). Every registry entrypoint
...
trust boundary. Migration 056 independently revalidates the complete v4
...
- clean `001 -> 056` apply and idempotent reapply;
...
2. apply migration `056` in staging;
