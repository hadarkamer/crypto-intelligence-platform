...
def _consensus_map(rows: List[Any]) -> Dict[str, Dict[str, Any]]:
    """Direction counts per symbol, deduplicated by canonical timeframe.

    The earlier implementation returned only the dominant side and its hit
    count. That caused minority-side alerts to inherit the dominant-side
    consensus score. This map keeps both LONG and SHORT counts so every alert
    is scored against its own direction.
    """
    ...
    for row in rows:
        ...
        if not symbol or timeframe not in TIMEFRAMES or side not in {"LONG", "SHORT"}:
            continue
        grouped[symbol][timeframe] = side

    result: Dict[str, Dict[str, Any]] = {}
    for symbol, by_tf in grouped.items():
        long_hits = sum(1 for side in by_tf.values() if side == "LONG")
        short_hits = sum(1 for side in by_tf.values() if side == "SHORT")
        total = len(by_tf)
        dominant = "LONG" if long_hits > short_hits else "SHORT" if short_hits > long_hits else None
        result[symbol] = {
            "symbol": symbol,
            "side": dominant,
...
def _consensus_hits_for_side(consensus_item: Dict[str, Any], side: str) -> int:
    return int(consensus_item.get("long_hits" if side == "LONG" else "short_hits", 0) or 0)
...
def _cluster_map(rows: List[Any]) -> Dict[tuple, Dict[str, Any]]:
    """Cluster Confidence by (symbol, side), 0..30.

    LONG and SHORT clusters are calculated independently. This prevents the
    dominant side's cluster count and liquidity multiplier from appearing in
    alerts for the opposite side.
    """
    ...
    for symbol, symbol_rows in grouped.items():
        for direction in ("LONG", "SHORT"):
            ...
            for row in symbol_rows:
                if _closest_side(row) != direction:
                    continue
                target = _target_for_side(row, direction)
                timeframe = str(_get(row, "timeframe", ""))
                ...
                directional_entries.append({
                    ...
                    "target": float(target),
                    "amount": max(0.0, _amount_for_side(row, direction)),
                })

            # One entry per canonical timeframe.
            directional_entries = sorted(
                {item["timeframe"]: item for item in directional_entries}.values(),
            ...
            if same_direction_count < 3:
                results[(symbol, direction)] = empty_result
                continue
            ...
            for item in directional_entries:
                distance_from_median_pct = abs(item["target"] - median_target) / median_target * 100.0
                if distance_from_median_pct <= CLUSTER_MEMBER_MAX_DISTANCE_PCT:
                    cluster_entries.append({**item, "distance_from_median_pct": distance_from_median_pct})
            ...
            deviations = [item["distance_from_median_pct"] for item in cluster_entries]
            ...
            entries_by_tf = {item["timeframe"]: item for item in cluster_entries}
            ...
            growth_score = (
                round(sum(transition_scores.values()) / len(transition_scores) * 10.0, 2)
...
def build_opportunities(
    ...
    # Defensive deduplication: one row per symbol/timeframe. The collection
    # audit already enforces this, but scoring must remain deterministic even
    # if callers pass duplicated rows directly.
    ...
    for row in rows:
        ...
        if symbol and timeframe in TIMEFRAMES:
            if key in deduped:
                duplicate_pairs.append(f"{symbol}/{timeframe}")
            deduped[key] = row
    if duplicate_pairs:
        print("[alert_engine] duplicate rows removed before scoring: " + ", ".join(sorted(set(duplicate_pairs))))
    ...
    for row in rows:
        ...
        consensus_hits = _consensus_hits_for_side(cons, side)
        ...
        cluster = clusters.get((symbol, side), {
            "count": 0,
        ...
        cluster_points = float(cluster.get("points", 0.0))
        ...
        if abs(component_sum - score) > 0.01:
            validation_errors.append(
                f"score mismatch: components={component_sum}, score={score}"
            )
        if consensus_hits > consensus_total:
            validation_errors.append(
                f"consensus hits exceed total: {consensus_hits}/{consensus_total}"
            )
        if int(cluster.get("count", 0) or 0) > consensus_hits:
            validation_errors.append(
                "cluster count exceeds same-direction consensus: "
                f"{cluster.get('count', 0)}>{consensus_hits}"
            )
        ...
        out.append({
            ...
            "calculation_valid": not validation_errors,
