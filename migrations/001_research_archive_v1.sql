...
-- the production runtime, or any recurring task.
-- Apply only after explicit approval and against the approved research database.
--
-- Timestamp semantics:
-- alert_time_utc is the decision/observation time of the signal, NOT the time a
-- Telegram API request happened to finish. Delivery lifecycle timestamps are
-- stored separately so historical joins never drift because Telegram was slow.

CREATE TABLE IF NOT EXISTS research_events (
    event_id BIGSERIAL PRIMARY KEY,
    schema_version TEXT NOT NULL,
    event_kind TEXT NOT NULL CHECK (event_kind IN ('ALERT', 'SIGNAL_STATE_CHANGE', 'DECISION_SAMPLE')),
    ...
    delivery_attempted_at_utc TIMESTAMPTZ,
...
CREATE INDEX IF NOT EXISTS idx_research_events_session_time
    ON research_events (runtime_session_id, alert_time_utc ASC);
...
CREATE TABLE IF NOT EXISTS research_alert_outcomes (
    ...
    time_to_closest_target_seconds INTEGER,
    time_to_target_seconds INTEGER,
    closest_target_price DOUBLE PRECISION,
