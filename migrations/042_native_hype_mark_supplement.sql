-- Explicit derived MARK measurements. Never a native Spot outcome or a new wave.
CREATE TABLE IF NOT EXISTS research_native_hype_mark_measurements (
    ...
    CHECK(measurement_payload->>'source_scope'='DERIVED_NATIVE_HYPE_MARK'),
    CHECK(measurement_payload->>'live_union_eligible'='false')
);
CREATE TABLE IF NOT EXISTS research_native_hype_mark_outcomes (
    ...
    FOREIGN KEY(event_id,adapter_version) REFERENCES research_native_hype_mark_measurements(event_id,adapter_version)
);
CREATE TABLE IF NOT EXISTS research_native_hype_mark_metrics (
    event_id BIGINT NOT NULL,
    adapter_version TEXT NOT NULL,
    window_minutes INTEGER NOT NULL CHECK(window_minutes IN(60,240,720,1440)),
    status TEXT NOT NULL,
