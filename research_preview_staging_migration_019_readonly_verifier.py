"""Read-only reconciler for PREVIEW staging migration 019.

This module is deliberately disconnected from every application runtime. It
accepts only the dedicated Render-internal staging database URL, forces a
read-only session and repeatable-read transaction, inspects the exact migration
019 catalog contract, and always rolls the transaction back. It never applies,
repairs or removes schema objects.
"""
...
VERIFIER_VERSION = "preview-staging-migration-019-readonly-verifier-v1"
MODE = "ONE_SHOT_RENDER_INTERNAL_POSTCOMMIT_READ_ONLY"

VERIFY_ENABLED_ENV = "FORMULA_PREVIEW_STAGING_MIGRATION_019_VERIFY"
...
_EXPECTED_ABSENT_CATALOG = {
    name: (False if isinstance(expected, bool) else 0)
    for name, expected in _EXPECTED_APPLIED_CATALOG.items()
}
...
_SAFETY = {
    ...
    "second_migration_trigger_allowed": False,
...
def resolve_configuration(environment: Mapping[str, Any]) -> Dict[str, Any]:
    """Validate the exact internal target without exposing its URL."""

    if not isinstance(environment, Mapping):
        raise ValueError("verifier environment must be a mapping")
    if not _is_enabled(environment, VERIFY_ENABLED_ENV):
        raise RuntimeError(
            f"refusing migration 019 verification: set {VERIFY_ENABLED_ENV}=1 explicitly"
        )
    for flag in _FORBIDDEN_ENABLED_FLAGS:
        if _is_enabled(environment, flag):
            raise RuntimeError(
                f"refusing migration 019 verification while {flag}=1"
            )
    for name in _FORBIDDEN_DATABASE_URLS:
        if str(environment.get(name, "")).strip():
            raise RuntimeError(
                f"refusing migration 019 verification while ambiguous {name} is configured"
            )

    database_url = str(environment.get(DATABASE_URL_ENV, "")).strip()
    if not database_url:
        raise RuntimeError(
            f"refusing migration 019 verification: {DATABASE_URL_ENV} is required"
        )
...
def _row_mapping(
    ...
    *,
    ...
    if isinstance(row, Mapping):
        ...
        if missing:
            raise RuntimeError(
                f"migration 019 read-only {phase} omitted required fields"
            )
    ...
    if isinstance(row, Sequence) and not isinstance(
        ...
        if len(row) != len(fields):
            raise RuntimeError(
                f"migration 019 read-only {phase} returned an unexpected row shape"
            )
        return dict(zip(fields, row))
    raise RuntimeError(f"migration 019 read-only {phase} returned an invalid row")


def _evaluate_identity(values: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    if values.get("current_schema") != "public":
        raise RuntimeError("migration 019 verifier search path did not select public")
    if values.get("transaction_read_only") != "on":
        raise RuntimeError("migration 019 verifier transaction is not read-only")
...
def _strict_mismatches(
    ...
    return [
        ...
        or type(values.get(name)) is not type(expected)
    ]
...
def _evaluate_catalog(
    ...
    applied_mismatches = _strict_mismatches(catalog, _EXPECTED_APPLIED_CATALOG)
    absent_mismatches = _strict_mismatches(catalog, _EXPECTED_ABSENT_CATALOG)
    applied_catalog_exact = not applied_mismatches
    absent_catalog_exact = not absent_mismatches

    reservation_rows = None
    consumption_rows = None
    row_mismatches: list[str] = []
    if row_counts is not None:
        reservation_rows = row_counts.get("reservation_row_count")
        consumption_rows = row_counts.get("consumption_row_count")
        for name, value in (
            ("reservation_row_count", reservation_rows),
            ("consumption_row_count", consumption_rows),
        ):
            if type(value) is not int or value < 0:
                raise RuntimeError("migration 019 row count is invalid")
            if value != 0:
                row_mismatches.append(name)

    if applied_catalog_exact and row_counts is None:
        raise RuntimeError("migration 019 exact catalog requires row verification")

    zero_rows = applied_catalog_exact and not row_mismatches
    if zero_rows:
        status = "APPLIED_VERIFIED"
        migration_applied: bool | None = True
        mismatches: list[str] = []
    elif absent_catalog_exact:
        status = "NOT_APPLIED"
        ...
        mismatches = sorted(set(applied_mismatches + row_mismatches))

    return {
        "status": status,
        "migration_019_applied": migration_applied,
        "catalog_matches_applied_contract": applied_catalog_exact,
        "catalog_matches_absent_contract": absent_catalog_exact,
        "zero_application_rows_verified": zero_rows,
        "reservation_row_count": reservation_rows,
        "consumption_row_count": consumption_rows,
        "catalog_mismatch_fields": mismatches,
        "manual_reconciliation_resolved": status != "PARTIAL_OR_CONFLICTING",
        "manual_intervention_required": status == "PARTIAL_OR_CONFLICTING",
        **catalog,
    }
...
def run_verifier(
    ...
    *,
    ...
    """Classify one exact read-only snapshot and unconditionally roll it back."""
    ...
    connection = connector(
        ...
        options=(
            ...
            "-c lock_timeout=2000 "
            "-c application_name=preview_staging_migration_019_readonly_verify"
        ),
    )
    began = False
    rolled_back = False
    try:
        ...
        identity_row = connection.execute(IDENTITY_SQL).fetchone()
        if identity_row is None:
            raise RuntimeError("migration 019 identity query returned no row")
        identity = _evaluate_identity(
            _row_mapping(identity_row, _IDENTITY_FIELDS, phase="identity")
        )

        catalog_row = connection.execute(CATALOG_SQL).fetchone()
        if catalog_row is None:
            raise RuntimeError("migration 019 catalog query returned no row")
        catalog = _row_mapping(catalog_row, _CATALOG_FIELDS, phase="catalog")

        row_counts = None
        read_only_queries = 2
        if not _strict_mismatches(catalog, _EXPECTED_APPLIED_CATALOG):
            count_row = connection.execute(ROW_COUNT_SQL).fetchone()
            if count_row is None:
                raise RuntimeError("migration 019 row-count query returned no row")
            row_counts = _row_mapping(
                count_row,
                _ROW_COUNT_FIELDS,
                phase="row count",
            )
        ...
        classification = _evaluate_catalog(catalog, row_counts)
        result = {
            ...
            **classification,
            ...
            "read_only_queries_executed": read_only_queries,
            "row_count_query_executed": row_counts is not None,
            **_SAFETY,
        }
...
def _failed_closed(error: BaseException) -> Dict[str, Any]:
    return {
        "verifier_version": VERIFIER_VERSION,
        "mode": MODE,
        "status": "POSTCOMMIT_VERIFY_FAILED_CLOSED",
        "error_type": type(error).__name__,
        "render_postgres_id": EXPECTED_RENDER_POSTGRES_ID,
        "database_url_exposed": False,
        "migration_019_applied": None,
        "manual_reconciliation_resolved": False,
...
def main() -> int:
    try:
        result = run_verifier(os.environ)
