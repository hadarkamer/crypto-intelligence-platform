...
import research_native_hype_perp_supplement as supplement
from research_native_hype_mark_supplement_selftest import event, membership, ENTRY
from research_native_hype_perp_supplement_selftest import perp_path as path


@unittest.skipUnless(os.environ.get("TEST_DATABASE_URL"), "TEST_DATABASE_URL required for PostgreSQL integration")
class NativePerpPostgresTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        import psycopg
    ...
    def setUp(self):
        self.schema = "native_perp_test_" + uuid4().hex
        ...
        self.conn.execute("CREATE TABLE research_events (" + ",".join(
            key + " " + types.get(key, "text") for key in shared.SOURCE_FIELDS) + ")")
        ...
        migration = Path(__file__).parent / "migrations/043_native_hype_perp_supplement.sql"
        self.conn.execute(migration.read_text())
        self.conn.execute(migration.read_text())
        self.conn.execute((Path(__file__).parent / "migrations/042_native_hype_mark_supplement.sql").read_text())
        original = event()
        self.conn.execute("INSERT INTO research_events VALUES (" + ",".join(
            "%s::jsonb" if key == "engine_snapshot" else "%s" for key in shared.SOURCE_FIELDS) + ")",
            tuple(shared.canonical(original[key]) if key == "engine_snapshot" else original[key]
                for key in shared.SOURCE_FIELDS))
        m = membership()
    ...
    def measure(self, minutes, price_path=None):
        source = shared._load(self.conn, [901])[0]
    ...
    def test_idempotence_open_advance_and_unchanged_native_source(self):
        source_before = shared._load(self.conn, [901])[0]
        ...
        self.assertEqual(self.scalar("SELECT count(*) FROM research_native_hype_perp_outcomes"), 32)
        self.assertEqual(self.scalar("SELECT count(*) FROM research_native_hype_perp_metrics"), 4)
        self.assertEqual(self.scalar("SELECT count(*) FROM research_native_hype_perp_metrics WHERE status='READY'"), 2)
        self.assertEqual(shared._load(self.conn, [901])[0], source_before)
        saved = self.conn.execute("SELECT measurement_payload FROM research_native_hype_perp_measurements").fetchone()["measurement_payload"]
    ...
    def test_changed_entry_or_original_source_is_rejected(self):
        ...
        with self.assertRaisesRegex(ValueError, "frozen PERP TRADE entry"):
        ...
        self.assertEqual(self.scalar("SELECT measurement_payload->>'entry_price' FROM research_native_hype_perp_measurements"), "100.0")

    def test_partial_write_rolls_back_and_retry_is_safe(self):
        self.conn.execute("ALTER TABLE research_native_hype_perp_metrics ADD CONSTRAINT injected_failure CHECK(window_minutes<>1440)")
        with self.assertRaises(Exception):
            supplement.write_measurement(self.conn, self.measure(1440))
        self.assertEqual(self.scalar("SELECT count(*) FROM research_native_hype_perp_measurements"), 0)
        self.assertEqual(self.scalar("SELECT count(*) FROM research_native_hype_perp_outcomes"), 0)
        self.conn.execute("ALTER TABLE research_native_hype_perp_metrics DROP CONSTRAINT injected_failure")
        self.assertTrue(supplement.write_measurement(self.conn, self.measure(1440)))
        self.assertEqual(self.scalar("SELECT count(*) FROM research_native_hype_perp_metrics WHERE status='READY'"), 4)
    ...
    def test_terminal_decision_and_ready_window_revisions_are_rejected(self):
        ...
        with self.assertRaisesRegex(ValueError, "frozen complete-window evidence"):
            supplement.write_measurement(self.conn, self.measure(240, changed))
        self.assertEqual(self.scalar("SELECT count(*) FROM research_native_hype_perp_metrics WHERE status='READY'"), 1)

    def test_perp_writes_never_enter_existing_mark_tables(self):
        self.assertTrue(supplement.write_measurement(self.conn, self.measure(1440)))
        ...
        self.assertEqual(self.scalar("SELECT count(*) FROM research_native_hype_mark_metrics"), 0)
        self.assertEqual(self.scalar("SELECT count(*) FROM research_native_hype_perp_outcomes"), 32)
        self.assertEqual(self.scalar("SELECT measurement_payload->>'source_scope' FROM research_native_hype_perp_measurements"), "DERIVED_NATIVE_HYPE_PERP")
