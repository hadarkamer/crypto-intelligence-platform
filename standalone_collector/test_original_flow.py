...
def sample():
    def zone(lo,hi):
        return {'low_price':lo,'high_price':hi,'relative_strength':'strong','confidence':'high'}
    return {'symbol':'BTC','analysis_mode':'visual_screenshot','model':'synthetic-model',
        'usage':{'input_tokens':1,'output_tokens':1},'scans':[{
            'timeframe':'12h','readable':True,'observed_symbol':'BTC',
            'observed_mode':'Symbol','observed_model':'Model 1','observed_timeframe':'12h',
            'blocking_condition':'none','current_price_estimate':100,
...
class OriginalFlowTests(unittest.TestCase):
    ...
    def test_capture_both_but_analyze_requested_image_once(self):
        ...
        with tempfile.TemporaryDirectory() as root,patch.dict(sys.modules,{'market_vision.coinglass_heatmap_capture':fake}),patch.object(scanner,'analyze_heatmap_images',side_effect=analyze):
            ...
            self.assertEqual(result['captured_at'],'2026-01-01T00:00:00Z')
        self.assertEqual(calls,[('capture',('12h','24h')),('analysis',('12h',))])
    def test_model_call_is_bounded_and_usage_retained(self):
        raw=sample();raw.pop('model');raw.pop('usage')
    ...
    def test_unreadable_result_does_not_create_output(self):
        def capture(directory,*,timeframes):
            ...
            for tf in timeframes:
                image=directory/f'coinglass_btc_heatmap_{tf}.png';image.write_bytes(b'\x89PNG\r\n\x1a\nsynthetic')
                result.append({'image':str(image),'timeframe':tf})
            return result
